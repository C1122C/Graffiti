...
class picture_info_dao
{
    ...
    //添加图片
    function picture_add($picture,$id,$path){
        $path="'".$path."'";
    ...
    function get_album($userid){
        ...
        $result=$this->sql->query($query);
        while($row = $result->fetchArray(SQLITE3_ASSOC) ) {
            $album[$index]['album'] = $row['ALBUM'];
            $query="SELECT * FROM PICTURE WHERE USER_ID=".$userid."AND ALNUM=".$row['ALBUM'].";";
            $result=$this->sql->query($query);
            while($row1 = $result->fetchArray(SQLITE3_ASSOC) ) {
                $album[$index]['pic'] = $row['PATH'];
            }
        }
        return $album;
    }
