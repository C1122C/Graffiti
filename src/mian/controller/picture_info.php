...
class picture_info
{
    ...
    function picture_del($userid,$picid){
        $result=$this->user_con->upload_del($userid,$picid);
        if($result){
    ...
    function picture_del_album($userid,$alid){
        $id=$this->user_con->upload_inq_idonly($userid,$alid);
        for($i=0;$i<count($id);$i++){
            $result=$this->picture_del($userid,$id[$i]);
            if($result==false){
                return false;
            }
        }
        return $this->user_con->album_del($userid,$alid);
    }
