...
class user_info
{
    ...
    function all_follow($userid){
        $tempstr=$this->user_dao->all_follow($userid);
        $follow=array();
    ...
    function announce_inq($userid,$type){
        ...
        $mission=array();
        for($i=0;$i<count($tempstr);$i++){
            $mis=$this->announce_con->announce_inq($tempstr[$i]['announce_id']);
            $mission[$i]=$mis;
        }
    ...
    function upload_inq($userid,$book){
        ...
        $tempstr=$this->my_tool->sort_by_date($tempstr);
        return $tempstr;
    }

    function upload_inq_idonly($userid){
        $result=$this->user_dao->upload_inq_byid($userid);
        $result=$this->my_tool->sort_by_date($result);
        return $result;
    }

    function change_mark($userid,$mark){
        return $this->change_mark($userid,$mark);
    }
    ...
    function all_group($userid){
        return $this->user_dao->all_group($userid);
    }
